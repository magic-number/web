...
export const placeholder = (val, length = 2, padstring = '0') => {
  const str = `${val}`;
  ...
  const holder = new Array(length - str.length).fill(padstring);
...
export const batchPromise = (ps, timeout = 5000) => Promise.race([
  Promise.all(ps),
